...
describe('PostIt-app:', () => {
  describe('', () => {   //	Unit test for the App Homepage route
  ...
  describe('Sign Up route', () => {
    it('should throw `statusCode 200` for every user`s who has registered',
    (done) => {
      request(app)
        .post('/signup')
        .send({
        ...
        .set('Accept', 'application/json')
          .end((err, res) => {
            ...
            expect(res.body.message).to.equal(
              'Registration successful and verification email sent to your mail'
            );
...
// Unit test for Use`s sign in route
  describe('sign in route', () => {
    it('should sign in a user`s who had signed up and return' +
    ' `User Signed in successfully`', (done) => {
      request(app)
      .post('/signin')
      .send({
        email: 'quduskunle@gmail.com',
    ...
    it('should throw `Not Found` and statusCode `404`' +
    ' if a user`s log in details is not found', (done) => {
      request(app)
      .post('/signin')
      .send({
  ...
  describe('allows', () => {
    it('registered user`s to create groups and return' +
    '`Group Created Successfully`', (done) => {
      request(app)
      .post('/group')
      .send({
        email: 'quduskunle@gmail.com',
  ...
  describe('should not allow', () => {
    it('non registered user`s to create groups and return' +
    ' `User`s not registered`', (done) => {
      request(app)
        .post('/group')
        .send({
  ...
  describe('should allow registered user`s', () => {
    it('to add registered friends to his/her group and should return' +
    ' `statusCode 200` and `Member added successfully`', (done) => {
      request(app)
  ...
  describe('should tell', () => {
    it('non registered user`s `User`s not Found` and return' +
    ' `statusCode 404`', (done) => {
      request(app)
      .post('/user/delete')
      .send({
  ...
  describe('should allow', () => {
    it('registered user`s to delete his/her account and should return' +
    ' `statusCode 200` and `User`s deleted successfully`', (done) => {
      request(app)
      .post('/user/delete')
      .send({
        email: 'quduskunle@gmail.com',
