...
describe('PostIt-app:', () => {
    ...
    (done) => {
      request(app)
        .post('/user/signup')
          ...
          .end((err, res) => {
            expect(res.statusCode).to.equal(200);
            expect(res.body.message).to.equal('Registration successful and \n' +
  ...
  describe('sign in route', () => {
    it('should sign in a user`s who had signed up\n' +
    ' and return `User Signed in successfully`', (done) => {
      request(app)
        .post('/user/signin')
        .send({
          email: 'quduskunle@gmail.com',
  ...
  describe('allows', () => {
    it('registered user`s to create groups and \n' +
    'return `Group Created Successfully`', (done) => {
      request(app)
        .post('/group')
        .send({
          email: 'quduskunle@gmail.com',
  ...
  describe('should not allow', () => {
    it('non registered user`s to create groups and \n' +
    'return `User`s not registered`', (done) => {
      request(app)
        .post('/group')
        .send({
  ...
  describe('should tell', () => {
    it('non registered user`s `User`s not Found` and\n' +
    'return `statusCode 404`', (done) => {
      request(app)
        .post('/user/delete')
        .send({
  ...
  describe('should allow', () => {
    it('registered user`s to delete his/her account and should\n' +
    'return `statusCode 200` and `User`s deleted successfully`', (done) => {
      request(app)
        .post('/user/delete')
        .send({
          email: 'quduskunle@gmail.com',
